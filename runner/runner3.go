...
func Configure() (err error) {
	// Set up logging.
	events = newLogger("ignite", "blue")
	warn = newLogger("ignite", "yellow")
	errs = newLogger("ignite", "red")
...
func build(event fsnotify.Event) error {
	eventName := event.Name
	eventType := event.Op.String()
	...
	if err != nil {
		//_, err = io.Copy(os.Stdout, bytes.NewReader(errBuf))
	...
	if len(errBuf) > 0 {
		saveBuildErrorsLog(string(append(errBuf, '\n')))
...
func Start() {
	...
	// Block indefinitely to continuously watch for file changes and rebuild as needed.
	<-make(chan int)
}
